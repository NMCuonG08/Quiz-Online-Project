...
export const PinCode = (options?: PinCodeOptions & ApiPropertyOptions) => {
  ...
  const decorators = [
    ...
    IsNotEmpty(),
    Matches(/^\d{6}$/, {
      message: ({ property }) => `${property} must be a 6-digit numeric string`,
    }),
...
export const ValidateHexColor = () => {
  const decorators = [
    IsHexColor(),
    Transform(({ value }) =>
      typeof value === 'string' && value[0] !== '#' ? `#${value}` : value,
    ),
  ];
...
export const ValidateBoolean = (
  options?: BooleanOptions & ApiPropertyOptions,
) => {
...
export const toEmail = ({ value }: IValue) =>
  ...
  return ValidateBy(
    {
      ...
      validator: {
        ...
        defaultMessage: () =>
          `$property must be a string in the format ${format}`,
      },
    },
    validationOptions,
  );
  ...
  return ValidateBy(
    {
      ...
      validator: {
        ...
        defaultMessage: buildMessage(
          (eachPrefix) =>
            eachPrefix + '$property must be an ip address, or ip address range',
          validationOptions,
        ),
      },
    },

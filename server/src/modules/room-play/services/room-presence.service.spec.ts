...
describe('RoomService realtime presence', () => {
  ...
  function createService() {
    let status: 'JOINED' | 'ACTIVE' | 'DISCONNECTED' | null = null;
    let currentParticipants = 0;
    const room = {
      id: roomId, room_code: 'ABC123', max_participants: 10,
      current_participants: 0, is_private: false,
    };
    const tx = {
      quizRoom: {
        findUnique: jest.fn(async () => ({ ...room, current_participants: currentParticipants })),
      ...
      roomParticipant: {
        findUnique: jest.fn(async () => status ? ({ id: 'participant-1', status }) : null),
        count: jest.fn(async () => status && ['JOINED', 'ACTIVE'].includes(status) ? 1 : 0),
        upsert: jest.fn(async () => { status = 'JOINED'; return { id: 'participant-1', status }; }),
        update: jest.fn(async ({ data }: any) => { status = data.status; return { id: 'participant-1', status }; }),
      },
    };
    const prisma = {
    ...
    Object.assign(service as any, {
      prisma,
      roomRepository: { findUnique: jest.fn(async () => ({ ...room, current_participants: currentParticipants })) },
      eventRepository: { joinUserToRoom: jest.fn(), emit: jest.fn() },
    });
    return { service, getCount: () => currentParticipants, getStatus: () => status };
  }

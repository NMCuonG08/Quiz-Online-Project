...
import type { Category as BaseCategory } from "@/modules/admin/categories/services/admin.category.service";
import {
    showError,
    showLoading,
    closeLoading,
...
const EditCourseCategory = () => {
    ...
    const onSubmit = async (data: CategoryFormData) => {
        try {
            ...
            if (!id) {
                showError("Course category is not loaded yet. Please try again.");
                return;
            }
            const payload = {
                name: data.name,
            ...
            closeLoading();
            if (res.success) {
                showSuccess("Course category updated successfully");
                router.push("/admin/course-categories");
            } else {
                const errObj = (res as { error?: unknown }).error;
                const msg = formatErrorMessageWithDetails(errObj, "Failed to update course category");
            ...
            closeLoading();
            showError("Failed to update course category");
        }
    };
        ...
        <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-[#122031] dark:shadow-card sm:p-7.5">
            ...
            >
                <div className="flex flex-col gap-5.5">
                    ...
                    <InputGroup
                        label="Slug"
                        placeholder="course-category-slug"
                        type="text"
                        name="slug"
                    ...
                    <Select
                        ...
                        items={[
                            ...
                            ...categories
                                .filter((c) => c.slug !== watchedValues.slug)
                                .map((c) => ({
                                    value: String(c.id),
                                    label: c.name,
                                })),
